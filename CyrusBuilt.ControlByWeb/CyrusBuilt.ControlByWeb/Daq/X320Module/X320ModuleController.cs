...
namespace CyrusBuilt.ControlByWeb.Daq.X320Module
{
    ...
    /// </summary>
    public class X320ModuleController : IDisposable
    {
        #region Fields
        ...
        /// </param>
        public X320ModuleController(IPAddress ip) {
            this._ip = ip;
        }
        ...
        /// </exception>
        public X320ModuleController(IPAddress ip, Int32 port) {
        ...
        /// </param>
        public X320ModuleController(IPEndPoint endpoint) {
            if (endpoint != null) {
        ...
        /// </exception>
        private X320Event GetEventFromXmlNode(XmlNode node) {
            if (node == null) {
                throw new ArgumentNullException("node");
            }

            X320Event evt = new X320Event();
            String itemText = String.Empty;
            Int32 value = 0;
            DateTime time = DateTime.MinValue;
            Double duration = 0.0;
            X320Action action = X320Action.None;
            ...
            if (!String.IsNullOrEmpty(itemText)) {
                action = X320Utils.GetActionFromString(itemText);
        ...
        /// <paramref name="node"/> cannot be null.
        /// </exception>
        private X320Diagnostics GetDiagsFromXmlNode(XmlNode node) {
            if (node == null) {
                throw new ArgumentNullException("node");
            }

            X320Diagnostics diags = new X320Diagnostics();
            String itemText = String.Empty;
            Int32 value = 0;
            Double temp = 0.0;
            Double volts = 0.0;
        ...
        /// <paramref name="node"/> cannot be null.
        /// </exception>
        private X320State GetStateFromXmlNode(XmlNode node) {
            if (node == null) {
                throw new ArgumentNullException("node");
            }
            ...
            // Get relay states (io1state and io2state).
            for (Int32 r = 1; r <= X320Constants.TOTAL_IO; r++) {
                rel = null;
                relState = RelayState.Off;
                iValue = 0;
                attrib = String.Format("io{0}state", r.ToString());
                ...
                // High times.
                attrib = String.Format("hightime{0}", r.ToString());
                itemText = Common.GetNamedChildNode(node, attrib).InnerText;
                if (!String.IsNullOrEmpty(itemText)) {
                    if (Double.TryParse(itemText, out dValue)) {
                        state.ChangeRelayHighTime(r, dValue);
                    }
                }

                // Transition count.
                attrib = String.Format("rawCount{0}", r.ToString());
                itemText = Common.GetNamedChildNode(node, attrib).InnerText;
                if (!String.IsNullOrEmpty(itemText)) {
                    if (Int32.TryParse(itemText, out iValue)) {
                        state.ChangeRelayTransitionCount(r, iValue);
                    }
                }
            }
            ...
            // Get raw frequency.
            dValue = 0.000;
            itemText = Common.GetNamedChildNode(node, "rawFrequency").InnerText;
            if (!String.IsNullOrEmpty(itemText)) {
                if (Double.TryParse(itemText, out dValue)) {
                    state.ChangeRawFrequency(dValue);
                }
            }

            // Get Scaled frequency.
            dValue = 0.000;
            itemText = Common.GetNamedChildNode(node, "frequency").InnerText;
            if (!String.IsNullOrEmpty(itemText)) {
                if (Double.TryParse(itemText, out dValue)) {
                    state.ChangeScaledFrequency(dValue);
                }
            }

            // Get frequency alarm condition.
            iValue = 0;
            itemText = Common.GetNamedChildNode(node, "freqAlrm").InnerText;
            if (!String.IsNullOrEmpty(itemText)) {
                if (Int32.TryParse(itemText, out iValue)) {
                    condition = Common.GetAlarmCondition(iValue);
                    state.SetFrequencyAlarmCondition(condition);
                }
            }

            // Get analog input states.
            for (Int32 i = 1; i <= X320Constants.TOTAL_ANALOG_INPUTS; i++) {
                dValue = 0.00;
                ai = null;
                attrib = String.Format("an{0}state", i.ToString());
                itemText = Common.GetNamedChildNode(node, attrib).InnerText;
                if (!String.IsNullOrEmpty(itemText)) {
                    if (Double.TryParse(itemText, out dValue)) {
                        ai = new AnalogInput(dValue);
                        state.AssignAnalogInput(i, ai);
                    }
                }

                // Alarm state.
                iValue = 0;
                condition = AlarmConditions.Normal;
                attrib = String.Format("an{0}Alrm", i.ToString());
                itemText = Common.GetNamedChildNode(node, attrib).InnerText;
                if (!String.IsNullOrEmpty(itemText)) {
                    if (Int32.TryParse(itemText, out iValue)) {
                        state.AssignAnalogInputAlarmCondition(i, condition);
                    }
                }
            }
            ...
            if (!String.IsNullOrEmpty(itemText)) {
                if (itemText.Equals("C")) {
                    state.ChangeTemperatureUnitOfMeasure(TemperatureUnits.Celcius);
                }
                else {
                    state.ChangeTemperatureUnitOfMeasure(TemperatureUnits.Fahrenheit);
                }
            }
            ...
            for (Int32 s = 1; s <= X320Constants.TOTAL_SENSOR_INPUTS; s++) {
                ...
                if (!String.IsNullOrEmpty(itemText)) {
                    if (Double.TryParse(itemText, out dValue)) {
                        si = new SensorInput(dValue);
                        state.AssignSensorInput(s, si);
                    }
                }
                ...
                iValue = 0;
                condition = AlarmConditions.Normal;
                attrib = String.Format("s{0}Alrm", s.ToString());
                itemText = Common.GetNamedChildNode(node, attrib).InnerText;
                if (!String.IsNullOrEmpty(itemText)) {
                    if (Int32.TryParse(itemText, out iValue)) {
                        condition = Common.GetAlarmCondition(iValue);
                        state.AssignSensorAlarmCondtion(s, condition);
                    }
                }
            }

            // Get external vars.
            for (Int32 e = 1; e <= X320Constants.TOTAL_EXT_VARS; e++) {
                dValue = 0.00;
                attrib = String.Format("extvar{0}", e.ToString());
                itemText = Common.GetNamedChildNode(node, attrib).InnerText;
                if (!String.IsNullOrEmpty(itemText)) {
                    if (Double.TryParse(itemText, out dValue)) {
                        state.SetExternalVariableValue(e, dValue);
                    }
                }
            }
        ...
        /// </summary>
        /// <returns>
        /// If successful, a <see cref="X320Diagnostics"/> state object
        ...
        /// that is not understood by this method.
        /// </exception>
        public X320Diagnostics GetDiagnostics() {
        ...
        /// that is not understood by this method.
        /// </exception>
        public X320Event GetEvent(Int32 id) {
        ...
        /// that is not understood by this method.
        /// </exception>
        public X320EventCollection GetEvents() {
            X320Event evt = null;
            X320EventCollection evtColl = new X320EventCollection();
        ...
        public void BeginPollCycle() {
            ...
            this._pollThread.Name = "X320StatePoller";
        ...
        /// that is not understood by this method.
        /// </exception>
        public void SetState(X320State state) {
            if (state == null) {
            ...
            X320State currentState = this.GetState();
            ...
            for (Int32 i = 1; i <= X320Constants.TOTAL_RELAYS; i++) {
                ...
                switch (i) {
                    case 1:
                        ...
                        if (currentState.TransitionCount1 != state.TransitionCount1) {
                            counter = 1;
                            counterVal = state.TransitionCount1;
                        }
                        break;
                    case 2:
                        ...
                        if (currentState.TransitionCount2 != state.TransitionCount2) {
                            counter = 2;
                            counterVal = state.TransitionCount2;
                        }
                        break;
                }
                ...
                // Construct the counter command if changing states.
                if ((counter > 0) && (counter < 3)) {
                    command = String.Format("count{0}={2}", counter.ToString(), counterVal.ToString());
                    command += "&noReply=1 HTTP/1.1\r\n\r\n";
                    if (this._authEnabled) {
                        command = Common.AppendAuthToCommand(command, this._password);
                    }
                    this.SendCommand(command);
            ...
            // TODO As of 7/10/2012, the manual states that "Commands can be
            // sent to the X-320 to control the outputs, counters, and external
            // variables". However, it only provides documentation on setting
            // the counters and the relays (assumes output mode). So.......
            // can we set the state of the analog inputs or sensor inputs?
            // Can we set the external variables? Can we set the alarm states?
            // Or are alarms read-only?
            currentState = null;
        }
        ...
        public void SwitchRelayOn(Int32 relayNum) {
            if ((relayNum < 1) || (relayNum > X320Constants.TOTAL_RELAYS)) {
                String err = String.Format("Must be a value 1 - {0}.",
                                            X320Constants.TOTAL_RELAYS.ToString());
